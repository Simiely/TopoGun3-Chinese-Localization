...
2. 右键 `install.ps1` → 「使用 PowerShell 运行」
   （或在命令行执行：`powershell -ExecutionPolicy Bypass -File install.ps1`）。
3. 重新启动 TopoGun 3。

脚本会先把 `%APPDATA%\TopoGun3` 备份到 `TopoGun3_backup_<时间戳>`，再以
**合并**方式复制中文文件，**保留你自己的 `license.json`** 等个人文件，不会丢失授权。

### 方法二：手动复制

1. 完全退出 TopoGun 3。
2. 备份：把 `%APPDATA%\TopoGun3` 复制为 `TopoGun3.bak`。
3. 把本仓库的 `TopoGun3\` 文件夹内容**合并复制**到 `%APPDATA%\TopoGun3`
   （覆盖同名文件即可，**不要删除**原有的 `license.json`）。
4. 重新启动 TopoGun 3。

> 提示：`%APPDATA%` 通常对应 `C:\Users\<你的用户名>\AppData\Roaming`。
> 若程序提示「重置 UI」后中文消失，是因为程序用 `defaults/` 模板还原；本仓库已
> 一并汉化 `defaults/`，重新运行安装脚本即可。
